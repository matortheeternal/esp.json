...
let generateRecordDef = function(signature, game, def) {
    let filePath = path.resolve(__dirname, 'data', game, `${signature}.json`);
    fs.writeFileSync(filePath, def);
};

let addDef = (key, def) => defs[key] = def;

let addSubrecordDef = function(signature, def) {
    defs[signature] = subrecord(signature, def);
};
...
module.exports = {
    generateRecordDef, addDef, addSubrecordDef, getDef,
    req, subrecord, subrecordArray, multiArray, multiStruct,
    struct, array, union,
